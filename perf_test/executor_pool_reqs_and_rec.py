...
def clean_db(db: neo4j.Driver):
	db.execute_query("MATCH (p) DETACH DELETE (p)")

def get_and_sync_list(db: neo4j.Driver, degrees: int, endpoint: str, key: str, query: str, query_key: str = None, **query_args):
	print("starting get_and_sync_list")
	data = get_json_list(endpoint, lambda x: x[key])
	for x in data:
		records = []
		if query_key != None:
		...
		u = records[0].value()
		if u['visited'] == 0:
			executor.submit(search, u['name'], db, degrees - 1)
	print("ending get_and_sync_list")

def search(start_acc: str, db: neo4j.Driver, degrees: int = 2):
	...
	if degrees > 0:
		executor.submit(get_and_sync_list, db, degrees, f"users/{start_acc}/followers", "login", """
		...
		executor.submit(get_and_sync_list, db, degrees, f"users/{start_acc}/following", "login", """
			MATCH (u1:User {name: $uname})
			...
			if r['visited'] <= 1:
				...
				for fork in forks:
					...
					if r['visited'] == 0:
						...
						executor.submit(search, fork.split("/")[0], db, degrees - 1)

				executor.submit(get_and_sync_list, db, degrees, f"repos/{repo}/contributors", "login", """
					MATCH (r:Repo {name: $rname})
					MERGE (u:User {name: $uname})
					...
					RETURN u
				""", "uname", rname=repo)
				executor.submit(get_and_sync_list, db, degrees, f"repos/{repo}/stargazers", "login", """
					MATCH (r:Repo {name: $rname})
					MERGE (u:User {name: $uname})
					ON CREATE SET u.visited = 0
					CREATE (u)-[:STARRED]->(r)
...
if __name__ == "__main__":
	start = time.time()
	db = neo4j.GraphDatabase.driver(os.getenv("DB_URI"), auth=(os.getenv("DB_USER"), os.getenv("DB_PASS")))
