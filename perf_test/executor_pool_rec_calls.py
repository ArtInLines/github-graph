...
def search(start_acc: str, db: neo4j.Driver, executor: ProcessPoolExecutor, degrees: int = 2):
	...
	if degrees > 0:
		...
		for follower in followers:
			...
			u = records[0].value()
			if u['visited'] == 0:
				executor.submit(search, u['name'], db, executor, degrees - 1)
			...
			if r['visited'] <= 1:
				...
				for fork in forks:
					...
					if r['visited'] == 0:
						...
						executor.submit(search, fork.split("/")[0], db, executor, degrees - 1)
